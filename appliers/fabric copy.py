...
def unet_forward_with_cached_hidden_states(
    ...
    def get_weights(hiddens, cached_hiddens, weight, module):
        ...
        return attn_with_weights(module, hiddens, encoder_hidden_states=combined_hs, weights=weights)
...
def unet_forward_with_cached_hidden_states(
    ...
    def get_weights(hiddens, cached_hiddens, weight, module):
        ...
        if hasattr(module, 'get_attention_scores'):
            print("MyClass contains the method 'get_attention_scores'.")
        else:
            print("MyClass does not contain the method 'get_attention_scores'.")
        module.old_get_attention_scores=module.get_attention_scores
        def new_get_attention_scores(self, query, key, attention_mask=None):
            attention_probs = self.olg_get_attention_scores(query, key, attention_mask)
            if weights is not None:
                if weights.shape[0] != 1:
                    weights = weights.repeat_interleave(module.heads, dim=0)
                attention_probs = attention_probs * weights[:, None]
                attention_probs = attention_probs / attention_probs.sum(dim=-1, keepdim=True)
            return attention_probs
        module.get_attention_scores=partial(new_get_attention_scores, module)
        print(12121212, module.processor.__class__.__name__)
        return module.processor(module, hiddens, encoder_hidden_states=combined_hs)
    ...
    for module in all_modules:
        if hasattr(module.attn1, "old_forward"):
            module.attn1.forward = module.attn1.old_forward
            module.attn1.get_attention_scores=module.attn1.old_get_attention_scores
            del module.attn1.old_forward
            del module.attn1.old_get_attention_scores
